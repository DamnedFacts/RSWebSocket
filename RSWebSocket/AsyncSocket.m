...
- (unsigned long)readLengthForTerm;

- (unsigned long)prebufferReadLengthForTerm;
...
- (NSString *)addressHost:(CFDataRef)cfaddr
{
	...
	const char *pStr = inet_ntop (pSockAddr->sa_family, pAddr, addrBuf, (unsigned int) sizeof(addrBuf));
	...
	if(theCurrentRead != nil && theReadStream != NULL)
	{
		...
		while(!done && !socketError && !maxoutError && [self hasBytesAvailable])
		{
			...
			if(theCurrentRead->term != nil)
			{
				...
				{
					unsigned long maxToRead = [theCurrentRead readLengthForTerm];
				...
				{
					didPreBuffer = YES;
					unsigned long maxToRead = [theCurrentRead prebufferReadLengthForTerm];
